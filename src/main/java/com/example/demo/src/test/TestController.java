...
public class TestController {
    ...
    @ResponseBody
    @GetMapping("/log")
    public BaseResponse<String> logTest() {
        log.info("로그 테스트");
        return BaseResponse.onSuccess("Success Test");
    }
    ...
    public BaseResponse<String> createMemo(@Validated @RequestBody MemoDto memoDto) {
        ...
        return BaseResponse.onSuccess("생성 성공!!");
    }
    ...
    @ResponseBody
    @GetMapping("/memos")
    public BaseResponse<List<GetMemoDto>> getMemos(@RequestParam int startPage) {
        List<GetMemoDto> getMemoDtoList = testService.getMemos(startPage);
        return BaseResponse.onSuccess(getMemoDtoList);
    }
